...
- **Folded `spec-interview` into `plan`'s INTERVIEW mode.** Closes [#101](https://github.com/ckallum/calsuite/issues/101) — superset of [PR #112](https://github.com/ckallum/calsuite/pull/112)'s merge. PR #112 landed independently on `main` while this branch was open; both targeted the same fold (`spec-interview` competed directly with `/plan interview`). PR #112 wrote the unique content (`performance budgets`, `accessibility considerations` topic dimensions, plus the "Write it as a definitive spec, not a discussion document" stance line in Step 5) into `skills/plan/SKILL.md` because INTERVIEW mode was still inline at the time. After merging origin/main, this branch ports the same three additions into `skills/plan/references/interview.md` where INTERVIEW mode now lives post-split. Net result is identical content with no behavioural drift between the pre-split and post-split worlds. Historical references in `CHANGELOG.md` (2.16 / 2.31) preserved.
- **Stripped `version:` field from all 19 skills that carried it.** Closes [#106](https://github.com/ckallum/calsuite/issues/106) (partial scope — see the issue for what got investigated and dropped). The field appeared on 19 skills (`babysit-pr`, `debug`, `execute`, `flow`, `improve-architecture`, `learn`, `lint-rule-gen`, `plan`, `plan-ceo`, `prevent`, `qa`, `receiving-pr-feedback`, `retro`, `review`, `ship`, `skill-builder`, `sweep-issues`, `worktrees`, `zoom-out`) but not the other 16 — and the installer doesn't parse it, Claude Code doesn't parse it, so it was cosmetic metadata that just created drift. Removed. The `arguments:` vs `argument-hint:` "drift" the audit also flagged turned out not to be drift — they serve different purposes (prose docs vs autocomplete shape), so left alone.

> _Note: the rolled-in commit message references `#100` for the spec-interview merge — that's wrong. The correct issue is #101. Parallel `gh issue create` calls from `/sweep-issues` returned URLs in non-deterministic completion order (second occurrence — the previous PR hit the same race condition, per the note in 2.34). Fixed in `skills/sweep-issues/SKILL.md` Step 4 — capture the URL per-candidate, run creates serially._
...
- **`skills/sweep-issues/SKILL.md` Step 4 race condition.** Parallel `gh issue create` calls return URLs in non-deterministic completion order, breaking creation-order → title mapping. Documented serial execution as required and showed the URL-capture pattern.
...
- **Hook `args: string[]` exec form** — tracked as [#91](https://github.com/ckallum/calsuite/issues/91). The published `claude-code-settings.json` schema at schemastore.org does not yet expose an `args` field on hook entries, only `command: string`. Migrating without the schema confirmed would be guessing at a private API. Marginal win anyway (calsuite hook paths have no spaces or shell-special chars, so the current string form has no quoting issues). Revisit when the schema lands.
- **`/fewer-permission-prompts` overlap with new built-in `/less-permission-prompts`** — `/fewer-permission-prompts` is not owned by calsuite. Nothing in calsuite references either name. No action needed on our end.
