...
- **Folded `spec-interview` into `plan`'s INTERVIEW mode.** Closes [#101](https://github.com/ckallum/calsuite/issues/101). `spec-interview` directly competed with `/plan interview` — same description shape, less developed body. Added the two unique topic dimensions (`performance budgets`, `accessibility considerations`) to `skills/plan/references/interview.md`, then deleted `skills/spec-interview/` and removed it from `config/profiles.json` (both `base.skills` and `monorepo-root.skills`).
...
> _Note: the rolled-in commit message references `#100` for the spec-interview merge — that's wrong. The correct issue is #101. Parallel `gh issue create` calls from `/sweep-issues` returned URLs in non-deterministic completion order (second occurrence — the previous PR hit the same race condition, per the note in 2.34). Worth a small follow-up to either serialise the creates or rely on the returned URL→title map rather than creation order._
