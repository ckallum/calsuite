...
Run three signal-gated checks against the diff before drafting the PR body. Each warns but does not block (unless `.claude/ship-config.json` sets `strict: true`) and self-skips if its trigger condition isn't met:

- **Gate 1 — PR-size:** triggers when `added > 400` lines. Lists top files and suggests splitting on commit boundaries.
- **Gate 2 — Test-presence:** triggers when `code_additions > 50` and `new_tests == 0`. Optional strict mode via `.claude/ship-config.json` `criticalPaths` + `strict`.
- **Gate 3 — Spec-contract deviation:** triggers when `$SPEC_DIR` is non-empty. Flags MISSING (spec promises, diff lacks) and EXTRA (diff introduces, spec doesn't describe) classes; option B applies strikethrough + dated addendum to `design.md`/`tasks.md`.
