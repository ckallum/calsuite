...
**Agent I — Spec-contract deviation (signal-gated: `$SPEC_DIR` non-empty):** reads `$SPEC_DIR/design.md` + `tasks.md` and flags MISSING (spec promises, diff doesn't deliver) and EXTRA (diff builds, spec doesn't describe) items. Dispatch only if `$SPEC_DIR` is non-empty (branch slug matches `.claude/specs/<slug>/` or a spec with both `design.md` and `tasks.md` was found). Prompt in [references/agents.md#agent-i-spec-contract-deviation-signal-gated-spec_dir-non-empty](references/agents.md#agent-i-spec-contract-deviation-signal-gated-spec_dir-non-empty).
