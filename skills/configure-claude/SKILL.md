...
1. Resolve the calsuite checkout:
   ```bash
   calsuite_dir="${CALSUITE_DIR:-$HOME/Projects/calsuite}"
   ```
   If the current working directory equals `$calsuite_dir`, ask the user which project to configure. Otherwise, use the current working directory.
